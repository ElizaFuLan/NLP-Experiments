...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        CD_index = self.word_vocab.get('<CD>', None)
        NULL_index = self.word_vocab.get('<NULL>', None)
        ROOT_index = self.word_vocab.get('<ROOT>', None)
        UNK_index = self.word_vocab.get('<UNK>', None)

        stack_indices = [state.stack[-i - 1] if i < len(state.stack) else NULL_index for i in range(3)]
        buffer_indices = [state.buffer[-i - 1] if i < len(state.buffer) else NULL_index for i in range(3)]
        indices = []

        for idx in stack_indices + buffer_indices:
            if idx in [ROOT_index, NULL_index]:
                indices.append(idx)
            else:
                word = words[idx] if idx < len(words) and words[idx] is not None else '<UNK>'
                word_index = CD_index if re.match(r'^\d+$', word) else self.word_vocab.get(word.lower(), UNK_index)
    ...
    def get_output_representation(self, output_pair):  
        ...
        one_hot_output = np.zeros(len(self.output_labels), dtype=np.int32)
